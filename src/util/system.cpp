...
const char * const BITCOIN_CONF_FILENAME = "raycoin.conf";
const char * const BITCOIN_PID_FILENAME = "raycoind.pid";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Raycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Raycoin";
#else
    // Unix
    return pathRet / ".raycoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Raycoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Raycoin Core developers";
    }
