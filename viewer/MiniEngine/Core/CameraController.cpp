...
CameraController::CameraController( Camera& camera, Vector3 worldUp ) : m_TargetCamera( camera )
{
    ...
    m_HorizontalLookSensitivity = 3.0f;
    m_VerticalLookSensitivity = 3.0f;
    m_MoveSpeed = 50.0f;
    m_StrafeSpeed = 50.0f;
    m_MouseSensitivityX = 1.5f;
    m_MouseSensitivityY = 1.5f;
...
void CameraController::Update( float deltaTime )
{
    ...
    float panScale = (m_FineRotation ? 0.05f : 1.0f) * timeScale;
    ...
    yaw += GameInput::GetAnalogInput(GameInput::kAnalogMouseX) * m_MouseSensitivityX * panScale;
    pitch += GameInput::GetAnalogInput(GameInput::kAnalogMouseY) * m_MouseSensitivityY * panScale;
...
void CameraController::ApplyMomentum( float& oldValue, float& newValue, float deltaTime )
{
    ...
    if (blendedValue - newValue <= FLT_EPSILON) blendedValue = newValue;
    oldValue = blendedValue;
    newValue = blendedValue;
}

void CameraController::ResetMomentum()
{
    m_LastYaw = 0.0f;
    m_LastPitch = 0.0f;
    m_LastForward = 0.0f;
