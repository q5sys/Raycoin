...
namespace TextRenderer
{
    ...
    Color g_color(1.f, 1.f, 0.f);
    Color g_hiColor(1.f, 0.25f, 0.25f);
} // namespace TextRenderer
    ...
    : m_Context(CmdContext)
{
    ...
    m_ViewWidth = ViewWidth >= 0 ? ViewWidth : g_DisplayWidth;
    m_ViewHeight = ViewHeight >= 0 ? ViewHeight : g_DisplayHeight;
...
void TextContext::ResetSettings( void )
{
    ...
    m_ShadowOffsetX = 0.1f;
    m_ShadowOffsetY = 0.1f;
    m_PSParams.ShadowHardness = 1.0f;
    m_PSParams.ShadowOpacity = 1.0f;
    m_PSParams.TextColor = TextRenderer::g_color;

    m_VSConstantBufferIsStale = true;
    m_PSConstantBufferIsStale = true;
    m_TextureIsStale = true;

    SetFont( L"default", 14.0f );
}
...
UINT TextContext::FillVertexBuffer( TextVert volatile* verts, const char* str, size_t stride, size_t slen )
{
    ...
    {
        wchar_t wc = (stride == 2 ? *(wchar_t*)iter : *(unsigned char*)iter);
        iter += stride;
...
void TextContext::DrawFormattedString( const wchar_t* format, ... )
{
    ...
    va_list ap;
    va_start(ap, format);
    _vsnwprintf_s( buffer, 1024, 1023, format, ap );
...
void TextContext::DrawFormattedString( const char* format, ... )
{
    ...
    va_list ap;
    va_start(ap, format);
    vsnprintf_s( buffer, 1024, 1023, format, ap );
