...
namespace Graphics
{
    ...
    const char* ResolutionLabels[] = { "Custom", "1280x720", "1600x900", "1920x1080", "2560x1440", "3200x1800", "3840x2160" };
    //EnumVar TargetResolution("Graphics/Display/Native Resolution", k1080p, kNumPredefinedResolutions, ResolutionLabels);
    eResolution TargetResolution = k1080p;
    ...
    uint32_t g_NativeWidthCustom = 0;
    uint32_t g_NativeHeightCustom = 0;
    ...
    void SetNativeResolution(void)
    {
        ...
        default:
        case kCustom:
            NativeWidth = g_NativeWidthCustom;
            NativeHeight = g_NativeHeightCustom;
            break;
        case k720p:
            NativeWidth = 1280;
            NativeHeight = 720;
...
{
    if ((!width || !height) || (width == g_DisplayWidth && height == g_DisplayHeight)) return;
...
void Graphics::Initialize(void)
{
    ...
    if (!bUseWarpDriver)
    {
        int gpuSelect = g_gpuSelect;
        for (uint32_t Idx = 0; DXGI_ERROR_NOT_FOUND != dxgiFactory->EnumAdapters1(Idx, &pAdapter); ++Idx)
        {
            DXGI_ADAPTER_DESC1 desc;
            pAdapter->GetDesc1(&desc);
            if (desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE || !IsDirectXRaytracingSupported(pAdapter.Get())) continue;
            if (gpuSelect-- > 0) continue;

            if (SUCCEEDED(D3D12CreateDevice(pAdapter.Get(), D3D_FEATURE_LEVEL_11_0, MY_IID_PPV_ARGS(&pDevice))))
            {
                pAdapter->GetDesc1(&desc);
                Utility::Printf(L"D3D12 Raytracing (DXR) hardware found:  %s (%u MB)\n", desc.Description, desc.DedicatedVideoMemory >> 20);
                break;
            }
        }
        if (pDevice) g_Device = pDevice.Detach();
    }
    ...
    if (g_Device == nullptr)
    {
        ...
        {
            Utility::Print("Failed to find a D3D12 Raytracing (DXR) hardware adapter.\n");
            return;
        }
    }
#ifndef RELEASE
...
{
    //if last update is over a second old, reset frame time
    int64_t CurrentTick = SystemTime::GetCurrentTick();
    if (SystemTime::TimeBetweenTicks(s_FrameStartTick, CurrentTick) >= 1)
    {
        s_FrameStartTick = CurrentTick;
        s_FrameTime = 0;
    }
