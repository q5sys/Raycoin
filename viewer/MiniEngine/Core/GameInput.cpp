...
namespace GameInput
{
    bool g_waitForClick = false;
    bool g_waitForClickMouseOnly = false;
}
...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmZeroInputs(bool mouseOnly = false)
    {
        memset(&s_MouseState, 0, sizeof(DIMOUSESTATE2));
        if (!mouseOnly) memset(s_Keybuffer, 0, sizeof(s_Keybuffer));
    }
    ...
    void KbmUpdate()
    {
        ...
        {
            if (GameInput::g_waitForClick)
                KbmZeroInputs(GameInput::g_waitForClickMouseOnly);
            if (s_Mouse && !GameInput::g_waitForClick)
            {
                s_Mouse->Acquire();
                s_Mouse->GetDeviceState(sizeof(DIMOUSESTATE2), &s_MouseState);
            }
            if (s_Keyboard && (!GameInput::g_waitForClick || GameInput::g_waitForClickMouseOnly))
            {
                s_Keyboard->Acquire();
...
void GameInput::UnaquireKbm(bool mouseOnly)
{
    if (s_Mouse) s_Mouse->Unacquire();
    if (s_Keyboard) s_Keyboard->Unacquire();
    g_waitForClick = true;
    g_waitForClickMouseOnly = mouseOnly;
}