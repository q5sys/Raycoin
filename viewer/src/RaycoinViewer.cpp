...
NumVar g_mouseSensitivity("Application/Mouse Sensitivity", 1.3, 0.1, 10, 0.1);
...
LONG WINAPI HandleException(struct _EXCEPTION_POINTERS* apExceptionInfo)
{
    ...
    WCHAR* szAppName = L"Raycoin-dumps";
    WCHAR* szVersion = L"" RAYCOIN_VERSION_STRING;
    ...
    swprintf_s(szFileName, MAX_PATH, L"%s%s\\%s-%04d%02d%02d-%02d%02d%02d-%ld-%ld.dmp",
    ...
    ExpParam.ExceptionPointers = apExceptionInfo;
    ExpParam.ClientPointers = TRUE;

    HMODULE mhLib = LoadLibrary(L"dbghelp.dll");
    MINIDUMPWRITEDUMP MiniDumpWriteDump = (MINIDUMPWRITEDUMP)GetProcAddress(mhLib, "MiniDumpWriteDump");
    ...
    if (bMiniDumpSuccessful)    Utility::Printf(L"Error: Unhandled Exception. Created crash dump: %s\n", szFileName);
    else                        Utility::Printf(L"Error: Unhandled Exception. Failed to create crash dump.\n");
...
#endif
    GameCore::RunApplication(*this, L"Raycoin Viewer " RAYCOIN_VERSION_STRING, IDI_RAYCOIN);
}
...
void RaycoinViewer::resetTracePerSec()
{
    memset(_frameRates, 0, sizeof(_frameRates));
    memset(_frameRateTotals, 0, sizeof(_frameRateTotals));
    _frameCount = 0;
    _tracePerSec = 0;
}

bool RaycoinViewer::IsDone()
{
    //don't update framerate when verifying a single ray
    if (_verifyPos.x < 0)
    {
        //calc framerate here because the frame time is updated during Present, just before IsDone is called
        //only update a single small bucket per frame but average over a large history
        int bucket = Graphics::GetFrameCount() % framesAvgDim;
        _frameRates[bucket][(Graphics::GetFrameCount()/framesAvgDim) % framesAvgDim] = Graphics::GetFrameRate();
        _frameRateTotals[bucket] = 0;
        for (auto f : _frameRates[bucket]) _frameRateTotals[bucket] += f;

        float frameRateAvg = 0;
        for (auto f : _frameRateTotals) frameRateAvg += f;
        _frameCount = std::min(_frameCount+1, framesAvgDim*framesAvgDim);
        frameRateAvg /= _frameCount;
...
void RaycoinViewer::Update(float deltaT)
{
    ...
    if (g_traceLogCur != _traceLogCurLast)
    {
        if (g_traceLogCur)
        {
            TraceResult& result = _traceLog[g_traceLogCur-1];
            _seed = result.seed;
            _target = result.target;
            _verifyPos = result.pos;
            _blockHeight = result.blockHeight;
            _blockNonce = result.blockNonce;
            _blockExtraNonce = result.blockExtraNonce;
        }
        else
            _verifyPos = {-1,-1};
        g_mode = MODE_COMPUTE;
    ...
    _cameraController->SetMouseSensitivity(g_mouseSensitivity, g_mouseSensitivity);
...

    {
        ...
        if (g_mode == MODE_COMPUTE)
        {
            ...
            if (traceResults)
            {
                ...
                _error = "GPU failed to provide trace and hash results (possibly due to excessive OC / UV).";
        }
        else
            context.Finish();
...
void raycoinViewer_start()          { RaycoinViewer::inst.start(); }
bool raycoinViewer_hasAdapter()     { return RaycoinViewer::inst.hasAdapter(); }

void raycoinViewer_getMinedBlockHash(const char* header_hex, bool& valid, char hash_hex[65])
{
    CBlockHeader header;
    if (!DecodeHexBlockHeader(header, header_hex))
    {
        valid = false;
        memset(hash_hex, 'F', 64);
        hash_hex[64] = 0;
        return;
    }
    ...
    valid = hash.first;
    strcpy_s(hash_hex, 65, hash.second.GetHex().c_str());
}

void raycoinViewer_terminate()      { RaycoinViewer::inst.terminate(); }
#endif